...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if($row['username']!=$name){
        echo "<script>alert('username is not match!');</script>";
    }
    ...
    $stmt = $conn->prepare("INSERT INTO member (name, mobile_number, address, aadhar_number, aadhar_file,email) VALUES (?, ?, ?, ?, ?,?)");
    if (!$stmt) {
        die("Failed to prepare statement: " . $conn->error);
    }

    $stmt->bind_param("ssssbs", $name, $mobile_number, $address, $aadhar, $filecontent,$email);
...
<body>
    <center>
        <form id="forms" action="member.php" method="POST" class="form" enctype="multipart/form-data" onsubmit="return formvalidate(event)">
            
        <h1 class="space">Application Form</h1><div class="wrapper">
